...
def run_flask():
    # Render asigna dinámicamente un puerto en la variable de entorno PORT, por defecto usamos el 10000
...
async def reiniciar_mes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    conn = sqlite3.connect("sqlite3_mensual.db") # O trivia_mensual.db según corresponda
    conn = sqlite3.connect("trivia_mensual.db")
    cursor = conn.cursor()
...
def main():
    # Iniciar Flask en un hilo separado para que responda al puerto de Render
